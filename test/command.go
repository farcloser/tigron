...
func (gc *GenericCommand) FeedFunction(fun func() io.Reader) {
	gc.cmd.FeedFunction(fun)
}
...
func (gc *GenericCommand) Background() {
	...
	// XXX handle the error or let it propagate to Wait?
	_ = gc.cmd.Run()
}
	...
	if expect != nil {
		...
		case internal.ExitCodeTimeout:
			assert.Assert(gc.t, expect.ExitCode == internal.ExitCodeTimeout,
				"Command unexpectedly timed-out"+debug)
		case internal.ExitCodeCancelled:
			assert.Assert(gc.t, expect.ExitCode == internal.ExitCodeCancelled,
				"Command was cancelled"+debug)
		default:
